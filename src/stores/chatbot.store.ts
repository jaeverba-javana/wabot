...
export const useChatbotStore = defineStore("chatbot", () => {
	...
	function getDifferentFields<T>(
		original: T | unknown,
		changes: Partial<T> | unknown,
	): Partial<T> | undefined {
		...
		for (const key in changes) {
			...
			if (isOriginalObject && isChangesObject) {
				const nestedDifferences = getDifferentFields(
					originalValue,
					changesValue,
				);
	...
	const wholeNode = computed<FlowNode[]>(() =>
		nodes.value.map((v) =>
			modifiedNodes.value.find((va) => v._id === va._id) ?? v
		)
	);
	...
	const updateNode = (modifiedNode: PFlowNode) => {
		const index = _modifiedNodes.value.findIndex((v) =>
			v._id === modifiedNode._id
		);
	...
	appStore.isUserLoggedIn
		? fetchNodes()
		: appStore.events.on("dataFetched", fetchNodes);
